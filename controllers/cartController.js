...
const extractDaysFromRentDuration = (durationString) => {
  ...
  return 1; // Default to 1 day if format is unclear
};
...
const validateRentalDates = (startDate, endDate, product = null) => {
  ...
  if (product) {
    try {
      ...
      const availableDateObjects = actualAvailableDates.map(
        (dateStr) => new Date(dateStr)
      );

      const isAvailable = checkDateRangeAvailability(
        start,
        end,
  ...
  // Check if rental duration meets minimum requirement
  if (product && startDate && endDate) {
    const rentalDays = Math.ceil((end - start) / (1000 * 60 * 60 * 24));
    const minRentDuration =
      product.rentDuration && product.rentDuration.length > 0
        ? extractDaysFromRentDuration(product.rentDuration[0])
        : 1;

    if (rentalDays < minRentDuration) {
      return {
        valid: false,
        message: `Minimum rental duration is ${minRentDuration} days. You selected ${rentalDays} days.`,
      };
    }
  }
...
export const getCart = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).populate({
      ...
      model: "Product",
      select:
        "name price images description category stock rentDuration bookedDates",
    });
...
export const addToCart = async (req, res) => {
  try {
    ...
    if (!req.user || !req.user.id) {
      console.error(
        "Authentication failed: req.user or req.user.id is missing"
      );
  ...
  try {
    ...
    for (const item of cartItems) {
      ...
      if (item.startDate && item.endDate) {
        ...
        if (!dateValidation.valid) {
          console.warn(
            `Skipping cart item ${item.productId} due to invalid dates: ${dateValidation.message}`
          );
          continue;
        }
      }
      ...
      if (existingCartItem) {
        existingCartItem.quantity = Math.max(
          existingCartItem.quantity,
          item.quantity
        );
        if (item.startDate)
          existingCartItem.startDate = new Date(item.startDate);
